...
for type in "${types[@]}"; do
	...
	cat > "$dockerfile" <<-EOF
		...
		# docker build -t docker-dev:${type} -f Dockerfile.${type} .
		#
	...
	case "$type" in
		...
		*)
			...
			echo 'RUN curl -fSL "https://storage.googleapis.com/golang/go${GO_VERSION}.linux-'${type}'64.tar.gz" | tar xzC /usr/local' >> "$dockerfile"
